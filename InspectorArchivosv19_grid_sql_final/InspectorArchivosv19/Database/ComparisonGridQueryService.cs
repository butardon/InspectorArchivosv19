...
FROM public.comparison_grid_cache c WHERE {string.Join(" AND ",where)} ORDER BY {order}, c.row_id;";
        ...
        private static string Column(string x)=>x switch{
            "Candidatura"=>"c.candidatura_origen","Estado"=>"c.estado","CarpetaPadreOrigen"=>"c.carpeta_padre_origen","CarpetaPadreDestino"=>"c.carpeta_padre_destino","RutaOrigen"=>"c.origin_fullpath","RutaDestino"=>"c.destination_fullpath","Nombre"=>"c.nombre","Extension"=>"c.extension","TamanoOrigen"=>"c.origin_size","TamanoDestino"=>"c.destination_size","FechaModOrigen"=>"NULLIF(c.origin_lastwritetime,'')::timestamptz","FechaModDestino"=>"NULLIF(c.destination_lastwritetime,'')::timestamptz","HashOrigen"=>"c.origin_hash","HashDestino"=>"c.destination_hash","Fingerprint"=>"c.fingerprint","EsDuplicado"=>"CASE WHEN c.es_duplicado THEN 'Sí' ELSE 'No' END","NombrePc"=>"COALESCE(c.origin_nombrepc,c.destination_nombrepc)","RevisadoOrigen"=>"c.origin_revisado","RevisadoDestino"=>"c.destination_revisado",_=>null};
